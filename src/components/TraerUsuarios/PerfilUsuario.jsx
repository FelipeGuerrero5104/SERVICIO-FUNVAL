  ...
  const handleProfilePictureChange = (e) => {
    ...
    if (selectedFile) {
      ...
      if (!allowedTypes.includes(selectedFile.type)) {
        setSubmissionMessage({ type: 'error', text: 'Solo se permiten imágenes JPEG o PNG.' });
        setProfilePicture(null);
        setPreviewImage(null);
        return;
      }
      if (selectedFile.size > 5 * 1024 * 1024) {
        setSubmissionMessage({ type: 'error', text: 'La imagen no debe exceder los 5MB.' });
        setProfilePicture(null);
        setPreviewImage(null);
        return;
      }
  ...
  const handleProfilePictureUpload = async () => {
    ...
    try {
      ...
      if (status === 200 || status === 201) {
        setSubmissionMessage({ type: 'success', text: 'Foto de perfil actualizada exitosamente.' });
        setProfilePicture(null);
        setPreviewImage(null);
      }
    } catch (error) {
      if (error.response?.status === 401) {
        ...
        const errorMessage = error.response?.data?.message || error.response?.data?.detail || 'Error al subir la foto de perfil.';
      ...
      <div className="w-full max-w-5xl flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-1/3 bg-white shadow-xl rounded-lg overflow-hidden">
          ...
          <div className="p-6 flex flex-col items-center">
            <img
              src={previewImage || user?.profile_picture || 'https://placehold.co/150'}
              alt="Foto de perfil"
              className="w-32 h-32 rounded-full border-4 border-white shadow-xl object-cover"
            />
            <h3 className="mt-4 text-lg font-semibold text-gray-800 text-center">{fullName}</h3>
        ...
        <div className="w-full lg:w-2/3 bg-white shadow-xl rounded-lg overflow-hidden">
          <div className="bg-gradient-to-br from-blue-800 to-blue-900 h-24 flex items-center justify-center">
            <h2 className="text-white text-xl font-bold">Información Personal</h2>
          </div>
          ...
          {submissionMessage.text && (
            <div className="p-6">
              <p className={`text-center text-sm font-medium ${submissionMessage.type === 'success' ? 'text-green-600' : 'text-red-600'}`}>
                {submissionMessage.text}
              </p>
            </div>
          )}
